...
export const seedISO27002 = async () => {
  try {
    console.log('Starting ISO 27002 seeding...');

    const existingFramework = await FrameworkModel.findOne({ 
      displayName: iso27002FrameworkData.displayName 
    });

    let framework;
    if (existingFramework) {
      console.log('ISO 27002 framework already exists, using existing one...');
      framework = existingFramework;
    } else {
      framework = await FrameworkModel.create(iso27002FrameworkData);
      console.log('ISO 27002 framework created successfully');
    }
    ...
    if (existingControlsCount > 0) {
      console.log(`${existingControlsCount} controls already exist for ISO 27002 framework`);
      return {
        framework,
        controlsCreated: 0,
    ...
    console.log(`Successfully created ${controls.length} controls for ISO 27002 framework`);
    
    return {
      framework,
      controlsCreated: controls.length,
      message: 'ISO 27002 framework and controls seeded successfully'
    };

  } catch (error) {
    console.error('Error seeding ISO 27002:', error);
    throw error;
  }
};

export const removeISO27002 = async () => {
  try {
    ...
    if (framework) {
      ...
      console.log('ISO 27002 framework and controls removed successfully');
    } else {
      console.log('ISO 27002 framework not found');
    }
  } catch (error) {
    console.error('Error removing ISO 27002:', error);
    throw error;
  }
};